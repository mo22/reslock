...
- Disk is deliberately NOT a first-class lease field (decided v0.9.0): lease cleanup is PID-based, so a lease for persistent artifacts (model weights) would be auto-dropped on process exit while the bytes remain — immediate accounting drift. Transient disk (scratch, in-flight downloads) IS lease-shaped and already works as a free-form counter (`detect_disk_mb()` for capacity, `acquire(disk_data_mb=...)`, `shrink()` on cleanup). Persistent usage: capacity reporting + consumer-side eviction policy (gpu_orphans philosophy).
